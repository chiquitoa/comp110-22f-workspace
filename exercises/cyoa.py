...
def greet() -> None:
    ...
    player = input("Oh no! It seems you've been transformed into a frog while you slept! (or is it really 'Hell yeah?') (Press Enter)")
...
def brook() -> None:
    ...
    global points
    reaction: int = 0
    # Sorry there are only so many frog pun jokes out there, sadly
    ...
    points += reaction
    # Tadpoles reaction is based on how many points are earned from the joke.
    if reaction < 8:
...
def pool() -> int:
    """Procedure for thaking the pool option. Cute mayfly minigame."""
    snacky_choice: str = ""
    mayflies: int = 0
    ...
    if snacky_choice == "y":
        # Randomly chooses how many mayflies are caught
        mayflies = random.randint(6, 15)
        input(f"You leap into the water, catching an unfortunate mayfly along your arc. You proceed to repeatedly leap out of the water to catch the mayflies buzzing about overhead! You caught a whopping {mayflies} mayflies! (Press Enter)")
...
def mantis(points: int) -> int:
    """Function for the mantis option."""
    choice: str = ""
    # This "boss fight" essentially is the end of the game if the player passes the skill check. If not it's just a point earner
...
def trail(points: int) -> int:
    """Function for taking the trail option."""
    choice: str = ""
    are_you_sure: str = ""
    ...
    if choice == "1":
        if points < 30:
            ...
            # This option adds 100 points, which then exits the game from the main() function at a test for a points value greater than 100
            if are_you_sure == "y":
...
def main() -> None:
    ...
    exit: str = ""
    greet()
    # Loop terminates at 50 points, which engages the finish text
    while points < 50:
        if day == 1:
            ...
            input("You head outside once more, and are presented with a familiar sight: (Press Enter)")
